...
export default function UserProfilePage() {
  ...
  const { data: savedSearches, isLoading, error } = api.savedSearch.getMySearches.useQuery(
    undefined, // no input
    {
  ...
  const deleteAccountMutation = api.user.deleteAccount.useMutation({
    onSuccess: async (data) => {
      toast.success(data.message || "Cuenta eliminada correctamente. Serás desconectado.");
  ...
  const requestDataReportMutation = api.user.requestDataReport.useMutation({
    onSuccess: (data) => {
      toast.success(data.message || "Informe de datos solicitado. Revisa tu correo electronico.");
    },
    onError: (error) => {
  ...
  if (isPending || isLoading) {
    return <div className="container mx-auto p-4 text-center">Cargando perfil de usuario...</div>;
  }

  if (!session) {
    // This case should ideally be handled by the useEffect redirect,
    // but as a fallback or if redirect hasn't happened yet:
    return <div className="container mx-auto p-4 text-center">Redirigiendo al inicio de sesión...</div>;
  }

  if (error) {
    return <div className="container mx-auto p-4 text-center text-red-500">Error al cargar las búsquedas guardadas: {error.message}</div>;
  }

  const handleSearchClick = (search: SavedSearchItem, event?: React.MouseEvent) => {
    // Prevent navigation if the click was on the delete button area
    if (event && (event.target as HTMLElement).closest('.delete-button-class')) {
    ...
    if (search.priceFrom) params.append("priceFrom", search.priceFrom.toString());
    ...
    <div className="container mx-auto p-4">
      <Card className="mb-8">
        <CardHeader>
          ...
          <CardDescription>Aqui puedes ver tus ajustes de cuenta y busquedas guardadas.</CardDescription>
        </CardHeader>
        <CardContent>
        {session?.user && (
        ...
        <CardFooter className="flex flex-col space-y-2 md:flex-row md:space-y-0 md:space-x-2 p-6 pt-0">
          <Button
            variant="outline"
          ...
          >
            ...
            {requestDataReportMutation.isPending ? "Solicitando..." : "Solicitar Informe de Datos"}
          </Button>
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  ...
                  y todos tus datos asociados, incluidas las búsquedas guardadas.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
          ...
          {savedSearches.map((search) => (
            <AlertDialog key={`alert-${search.id}`}>
              <Card className="relative hover:shadow-lg">
                <div className="p-4 cursor-pointer" onClick={(e) => handleSearchClick(search, e)}>
                  <CardHeader className="p-0 pb-2">
                    <CardTitle className="text-lg">{search.name || `Búsqueda del ${new Date(search.createdAt).toLocaleDateString()}`}</CardTitle>
                  </CardHeader>
                  <CardContent className="p-0">
                    ...
                    <p className="text-xs text-gray-500">Guardada el: {new Date(search.createdAt).toLocaleString()}</p>
                  </CardContent>
                </div>
                <AlertDialogTrigger asChild>
                  <Button
                    variant="ghost"
                    size="icon"
                    className="absolute top-2 right-2 delete-button-class"
                    onClick={(e) => {
                ...
                <AlertDialogContent> {/* This is for deleting a single search */}
                  <AlertDialogHeader>
                    <AlertDialogTitle>¿Eliminar esta búsqueda guardada?</AlertDialogTitle>
                    <AlertDialogDescription>
                      Esta acción no se puede deshacer. Esto eliminará permanentemente tu búsqueda guardada: <br />
                      <strong>{search.name || `Búsqueda del ${new Date(search.createdAt).toLocaleDateString()}`}</strong>.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
                    <AlertDialogCancel>Cancelar</AlertDialogCancel>
